...
function server_start($name) {
	...
	// Check that server has a .exe
	if(is_file($user['home'].'/TerrariaServer.exe')) {
		...
		if(is_file($user['home'].'/server.properties')) {
			$properties = json_decode(file_get_contents($user['home'].'/server.properties'),true);
		} else {
			// File doesn't exist
			file_put_contents(
...
function server_players($name) {

	$user = user_info($name);

	$config = json_decode(file_get_contents($user['home']."/tshock/config.json"),true);
	if($config['RestApiEnabled']==true)
	{
		$buff = json_decode(file_get_contents("http://".KT_LOCAL_IP.":".$config['RestApiPort']."/v2/server/status"),true);

		$result = array(
			'current' => intval($buff['playercount']),
			'max' => intval($buff['maxplayers'])
		);
...
function server_running($name) {
	$result = stripos(`tmux ls`,KT_SCREEN_NAME_PREFIX.$name);
	if($result===false)
	{
		return false;
	}
	...
	//return (bool) stripos(`tmux ls`,KT_SCREEN_NAME_PREFIX.$user['user']);
}
